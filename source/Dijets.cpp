...
namespace Dijets
{
	float EtLab( const PseudoJet& jet, Double_t PzInLepton, Double_t PzInHadron, TLorentzVector electron )
	{
		TLorentzVector p;
		...
		OutputCand.SetOwner();
		float et = (static_cast<TLorentzVector*>(OutputCand.At(0))) -> Et();
		OutputCand.Clear();
		return et;
	}
	
	float EtaLab( const PseudoJet& jet, Double_t PzInLepton, Double_t PzInHadron, TLorentzVector electron )
	{
		TLorentzVector p;
		p . SetPxPyPzE( jet.px(), jet.py(), jet.pz(), jet.e() );
		TObjArray InputCand;
		...
		Dijets::BreitBoost( InputCand,  OutputCand, PzInLepton, PzInHadron, electron, 2 );
		OutputCand.SetOwner();
		float eta = (static_cast<TLorentzVector*>(OutputCand.At(0))) -> Eta();
		OutputCand.Clear();
		return eta;
	}
