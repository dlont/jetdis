...
//energy scale unc.
	if( fSettings->is_eval_enescale_syst() )
	{
		...
		for( itBreit = alljets_breit.begin(); itBreit != alljets_breit.end(); ++itBreit )
		{
			...
			if( et_lab > 10. ) (*itBreit) *= 0.99;//up:1.01; down:0.99
			if( et_lab < 10. ) (*itBreit) *= 0.97;//up:1.03; down:0.97
		}
		vector<PseudoJet>::iterator itLab;
		for( itLab = alljets_lab.begin(); itLab != alljets_lab.end(); ++itLab )
		{
			et_lab  =  (*itLab).perp();
			if( et_lab > 10. ) (*itLab) *= 0.99;//up:1.01; down:0.99
			if( et_lab < 10. ) (*itLab) *= 0.97;//up:1.03; down:0.97
		} 
	}
